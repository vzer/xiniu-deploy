...
netstat -lnp | grep $appport | awk '{print $7}' | sed 's/\/java/ /' | xargs kill -9
...
cd /xiniu/apps/$Service/$Version
if [ "$mode" = "yes" ]; then
autoconfig $Service-business-$Version* -c UTF-8 -s ssh://xiniu@10.173.83.161:/xiniu/antx.properties
autoconfig $Service-business-$Version*/lib/$Service-business-$Version*.jar -c UTF-8 -s ssh://xiniu@10.173.83.161:/xiniu/antx.properties
elif [ "$mode" = "no" ]; then
