...
import datetime
import subprocess
from config import PrePlatformServiceIp as ServiceIp
from config import PrePlatformWebIp as WebIp
from config import PrePlatformJobIp as JobIp
from config import user,password
from config import preplatserviceLocalPath as serviceLocalPath
from config import preplatserviceScriptFile as serviceScriptFile
from config import preplatwebScriptFile as webScriptFile
from config import preplatwebLocalPath as webLocalPath
from config import preplatjobLocalPath as jobLocalPath
from config import preplatjobScriptFile as jobScriptFile
from config import clientserviceLocalPath,clientwebLocalPath,clientjobLocalPath
...
#采用 paramiko 上传文件
def putFileWork(accout,passwd,host,localPath,remotePath,filename,scriptFile,restartFile=None):
    #检查并建立远端文件夹
    ...
    try:
        ...
        if restartFile!=None:
            sshFtp.put(localPath+restartFile,remotePath+restartFile)
            ...
            for key,value in ServiceIp[int(choice)].items():
                while True:
                    for value2 in range(len(value)):
                        printColor("green","%s-%s(%s:%s)"%(value2,key,value[value2][0],value[value2][1]))
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        printColor("purple","the Deployment of host:%s"%value[int(choice1)][0])
                        printColor("purple","the Deployment of port:%s"%value[int(choice1)][1])
                        ...
                        hostname=value[int(choice1)][0]
                        appport=value[int(choice1)][1]
                        putFileStatus=putFileWork(user,password,hostname,serviceLocalPath,clientserviceLocalPath,filename,serviceScriptFile)
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s %s %s %s'%(user,hostname,clientserviceLocalPath,serviceScriptFile,key,version,mode,clientjobLocalPath,appport),shell=True)
            ...
            for key,value in WebIp[int(choice)].items():
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        hostname=value[int(choice1)][0]
                        appport=value[int(choice1)][1]
                        putFileStatus=putFileWork(user,password,hostname,webLocalPath,clientwebLocalPath,filename,webScriptFile)
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s %s %s %s'%(user,hostname,clientwebLocalPath,webScriptFile,key,version,mode,clientwebLocalPath,appport),shell=True)
                            except subprocess.CalledProcessError,msg:
                                printColor('red',msg)
                                printColor("red","User name or password is incorrect, please check!!")
                        choice3=rawInputColor("yellow","do you want need deploy the next %s host?(yes/no/quit):"%key)
            ...
            for key,value in JobIp[int(choice)].items():
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        putFileStatus=putFileWork(user,password,value[int(choice1)],jobLocalPath,clientjobLocalPath,filename,jobScriptFile)
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s %s %s'%(user,value[int(choice1)],clientjobLocalPath,jobScriptFile,key,version,mode,clientjobLocalPath),shell=True)
                            except subprocess.CalledProcessError,msg:
                                printColor('red',msg)
                                printColor("red","User name or password is incorrect, please check!!")
                        choice3=rawInputColor("yellow","do you want need deploy the next %s host?(yes/no/quit):"%key)
