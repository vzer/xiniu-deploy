...
cd /xiniu/apps/$Service/$Version
if [ "$mode" = "yes" ]; then
autoconfig $Service-business-$Version* -c UTF-8 -s ssh://xiniu@172.16.0.54:/xiniu/antx.properties
autoconfig $Service-business-$Version*/lib/$Service-business-$Version*.jar -c UTF-8 -s ssh://xiniu@172.16.0.54:/xiniu/antx.properties
elif [ "$mode" = "no" ]; then
