...
from config import proplatwebrestartFile as webRestartFile
...
from config import clientserviceLocalPath,clientwebLocalPath,clientjobLocalPath,clientmeilocalPatch,clientmeiremotePatch
            ...
            for key,value in ServiceIp[int(choice)].items():
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        putFileStatus=putFileWork(user,password,value[int(choice1)],serviceLocalPath,clientserviceLocalPath,filename,serviceScriptFile)
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s %s %s'%(user,value[int(choice1)],clientserviceLocalPath,serviceScriptFile,key,version,mode,clientjobLocalPath),shell=True)
            ...
            for key,value in WebIp[int(choice)].items():
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        putFileStatus=putFileWork(user,password,value[int(choice1)],webLocalPath,clientwebLocalPath,filename,webScriptFile,webRestartFile)
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s %s %s'%(user,value[int(choice1)],clientwebLocalPath,webScriptFile,key,version,mode,clientwebLocalPath),shell=True)
                                local_dir="%s%s"%(clientmeilocalPatch,key)
                                remote_dir=clientmeiremotePatch
                                upload(local_dir=local_dir,remote_dir=remote_dir,hostname=value[int(choice1)],port=22,username=user,password=password)
                                subprocess.check_call('ssh %s@%s sh %s%s'%(user,value[int(choice1)],clientwebLocalPath,webRestartFile),shell=True)
                            except subprocess.CalledProcessError,msg:
                                printColor('red',msg)
                                printColor("red","User name or password is incorrect, please check!!")
                        choice3=rawInputColor("yellow","do you want need deploy the next %s host?(yes/no/quit):"%key)
